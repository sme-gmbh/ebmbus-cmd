...
bool EbmModbus::open()
{
    ...
    if (m_bus == NULL) {
        emit signal_newEntry(LogEntry::Error, "ModbusSystem", "Unable to open interface");
        return false;
    }

    if (modbus_connect(m_bus) == -1) {
        emit signal_newEntry(LogEntry::Info, "ModbusSystem", QString("Unable to connect to device: ") + QString(modbus_strerror(errno)));
        modbus_free(m_bus);
        return false;
    }
